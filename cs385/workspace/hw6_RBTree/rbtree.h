...
/*
* GDB to debug a program with arguments
* gdb --args ./gcd 24 6
*/
...
class RedBlackTree : public Tree {
    ...
    */
    void insert(const iterator &it, const std::pair<K, V> &key_value){
        ...
        RedBlackNode<K, V> *n = new RedBlackNode<K, V>(key, value);
        while (x != NULL){
            y = x;
            if (key == x->key()){
    ...
    */
    int height() const{
        return height(root_) - 1;//added -1 to fix vals
    }
    ...
    void right_rotate(Node<K, V> *y){
        if (y != NULL){
            ...
            if (x != NULL){
                ...
                if (y->parent() == NULL){
                    root_ = static_cast< RedBlackNode<K, V>* >(x);
                }
    ...
    */
    int height(Node<K, V> *node) const{
        int h = 0;
        if (node != NULL){
            h += 1 + std::max(height(node->left()), height(node->right()));
        }
        return h;
    }
    ...
    */
    size_t leaf_count(Node<K, V> *node) const{
        ...
        if (!(node == NULL)){
            ...
            l_count += leaf_count(node->right());
            l_count += leaf_count(node->left());
        }
        return l_count;
    }
    ...
    */
    int diameter(Node<K, V> *node) const{
        ...
        if (node != NULL){
            int maxD;
            if (node->left() != NULL){
                d += 1 + (height(node->left()) - 1);
            }
            if (node->right() != NULL){
                d += 1 + (height(node->right()) - 1);
            }
            maxD = diameter(node->left());
            if (maxD > d){
                d = maxD;
            }
            maxD = diameter(node->right());
            if (maxD > d){
                d = maxD;
            }
        }
    ...
    size_t width(Node<K, V> *node, size_t level) const{
        ...
        if (node != NULL){
            if (level == 0){
                ...
                --level;
                count += width(node->left(), level);
                count += width(node->right(), level);
            }
        }
        return count;
    }
    ...
    */
    size_t sum_levels(Node<K, V> *node, size_t level) const{
        ...
        if (node != NULL){
            count += level;
            ++level;
            count += sum_levels(node->left(), level);
            count += sum_levels(node->right(), level);
        }
        return count;
    }
    ...
    */
    size_t sum_null_levels(Node<K, V> *node, size_t level) const{
        size_t null_levels = 0;
        if (node == NULL){
            return level;
        }
        ++level;
        null_levels += sum_null_levels(node->left(), level);
        null_levels += sum_null_levels(node->right(), level);
        return null_levels;
    }
};
