...
int main(int argc, char *argv[]) {
	bool DEBUG = false;//turns on or off debug messages
    ...
    if (using_ints) {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            try {
            	//if(DEBUG)cout << "Inserting a value" << endl;
                rbt->insert(value, value);
            } catch (const tree_exception &te) {
                cerr << "Warning: " << te.what() << endl;
            }
        }
        if(DEBUG)cout << "Displaying stats" << endl;
        display_stats<int, int>(rbt);
        if(DEBUG)cout << "Deleting tree" << endl;
        cleanup:
            delete rbt;
    } else {
